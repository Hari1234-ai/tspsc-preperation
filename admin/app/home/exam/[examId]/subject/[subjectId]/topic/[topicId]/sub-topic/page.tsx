...
export default function TopicSubtopicsPage() {
  ...
  useEffect(() => {
    if (examId && subjectId && topicId) fetchData();
  ...
  const handleBulkAssign = async () => {
    ...
    try {
      await fetch(`${API_URL}/topics/${topicId}/subtopics/bulk`, {
        method: "POST",
  ...
  const handleUnassign = (subtopicId: string, e: any) => {
    ...
    setConfirmConfig({
      ...
      message: "Are you sure you want to decouple this sub-topic node from the chapter hierarchy?",
      onConfirm: async () => {
        setIsConfirmOpen(false);
        await fetch(`${API_URL}/syllabus/papers/${examId}/subjects/${subjectId}/topic/${topicId}/subtopics/${subtopicId}`, { method: "DELETE" });
        fetchData();
      }
    });
  ...
  return (
    <div className="font-sans max-w-6xl mx-auto text-white">
      ...
      <div className="text-[10px] font-black text-gray-500 mb-12 flex items-center gap-3 uppercase tracking-[0.2em]">
        ...
        <span className="text-gray-800">/</span>
        <Link href={`/home/exam/${examId}/subject/${subjectId}/topic`} className="hover:text-white transition-colors uppercase">{subject?.title}</Link>
        <span className="text-gray-800">/</span>
      ...
      <div className="group bg-[#0d1117] rounded-[3rem] p-12 mb-16 border border-gray-800 flex gap-10 items-center relative overflow-hidden">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4 mb-6">
            <span className="text-[10px] font-black uppercase py-1 px-4 bg-indigo-500/10 text-[#6366f1] rounded-full tracking-[0.2em] border border-indigo-500/10">Topic Node</span>
          ...
          <p className="text-md text-gray-400 leading-relaxed max-w-2xl font-bold italic">
            {topic.description || `"Link and arrange granular sub-topics for this chapter. These represent the specific modules where aspirants will consume content."`}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6 pb-20">
        {topic.subtopics.length === 0 ? (
           <div className="py-24 text-center bg-[#0d1117] rounded-[3rem] border border-gray-800 border-dashed">
             <p className="text-[#6366f1] font-black text-xl mb-2">No sub-topics linked to this node yet.</p>
             <p className="text-gray-500 font-bold italic">"Add granular modules like audio, video, or deep text research files to this chapter."</p>
           </div>
        ) : (
          topic.subtopics.map((st: any, i: number) => (
            <div 
              key={st.id} 
              onClick={() => router.push(`/home/exam/${examId}/subject/${subjectId}/topic/${topicId}/sub-topic/${st.id}/modules`)}
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500 font-bold italic opacity-60 group-hover:opacity-100 transition-opacity">
                  {st.description || "Theoretical chapters, research papers, and modular resource insights."}
                </p>
              </div>
          ...
          <div className="fixed top-0 right-0 h-full w-[520px] bg-[#0a0c10] border-l border-gray-800 z-[101] p-12 shadow-3xl animate-in slide-in-from-right duration-700 ease-out flex flex-col overflow-y-auto">
            ...
            <div className="mt-10 pt-10 border-t border-gray-900">
               ...
               <p className="text-center text-[10px] text-gray-600 font-bold mt-4 uppercase tracking-widest italic leading-relaxed">
                 "Sub-topics selected here will be integrated into the {topic.title} chapter."
               </p>
            </div>
          </div>
        </>
